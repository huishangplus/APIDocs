...
- 后台通知交互时，如果魔方收到商户的应答不符合规范或超时，魔方会判定本次通知失败，重新发送通知，直到成功为止（在通知一直不成功的情况下，通知的频率为：第一次立即通知、第二次 30秒、第三次 300秒、第四次 1800秒（再未收到成功响应时，会持续发通知，但最多只通知四次）），但魔方不保证通知最终一定能成功。
- 在订单状态不明或者没有收到魔方支付结果通知的情况下，建议商户主动调用魔方[【订单状态同步API】](http://apidocs.huishangplus.com/#/payguide/apisync)确认订单状态。
...
|rpid|请求id|64|O|11121751762513404899|标识此次请求的唯一id，无业务含义|
